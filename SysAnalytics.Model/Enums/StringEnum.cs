...
namespace SysAnalytics.Model.Enums
{
	...
	public class StringEnum
	{
        ...
        public static string GetEnumsForJson(string enumName)
        {
            var assembly = Assembly.GetAssembly(typeof(GeneralEnum));
            Type enumType = assembly.GetType(string.Format("{0}.{1}", assembly.GetName().Name, enumName));
            ...
            {
                j++;
                if (j > 1) enumBuilder.AppendFormat(";");
                enumBuilder.AppendFormat("{0}:{1}", Enum.GetName(enumType, i), i.ToString());
            }
            return enumBuilder.ToString();
        }

        public static object GetEnum(Type type, string stringValue)
        {
            object output = null;
            string enumStringValue = null;

            if (!type.IsEnum) throw new ArgumentException(String.Format("Supplied type must be an Enum.  Type was {0}", type.ToString()));
            foreach (var enumValue in type.GetEnumValues())
            {
                if (string.Compare(enumValue.ToString(), stringValue) == 0)
                {
                    output = enumValue;
                    break;
                }
